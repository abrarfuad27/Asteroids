...
def group_group_collide(group1, group2):
    ...
    copy2 = set([])
    for b in copy1:
        if group_collide(group2, b) == True:
            copy2.add(b)
            group1.discard(b)
            strike += 1
            explosion_sound.play()
    return int(len(copy2))
...
def draw(canvas):
    global time, started, rock_group, lives, score, strike, missile_group, my_ship
...
def process_sprite_group(canvas, sprite_set):
    ...
    sprite_set2 = set(sprite_set)
    for b in sprite_set2:
        if b.update() == True:
            sprite_set.discard(b)
